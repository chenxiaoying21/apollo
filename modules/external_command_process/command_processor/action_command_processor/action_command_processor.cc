...
bool ActionCommandProcessor::Init(const std::shared_ptr<cyber::Node>& node) {
  ...
  CHECK(config.output_command_name().size() > 1);
  planning_action_writer_ = node->CreateWriter<apollo::planning::PadMessage>(
...
bool ActionCommandProcessor::GetCommandStatus(uint64_t command_id,
