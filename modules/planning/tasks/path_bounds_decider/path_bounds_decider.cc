...
bool PathBoundsDecider::Init(
  ...
  return Decider::LoadConfig<PathBoundsDeciderConfig>(&config_);
}
...
Status PathBoundsDecider::Process(
  ...
  if (plan_pull_over_path) {
    ...
    if (!ret.ok()) {
      ...
      candidate_path_boundaries.emplace_back(
          ...
          FLAGS_path_bounds_decider_resolution, pull_over_path_bound_pair);
  ...
  if (config_.enable_smarter_lane_change() &&
    ...
    candidate_path_boundaries.emplace_back(
        ...
        FLAGS_path_bounds_decider_resolution, lanechange_path_bound_pair);
...
Status PathBoundsDecider::GeneratePullOverPathBound(
  ...
  if (curr_idx < 0) {
    ...
    pull_over_status->set_length_front(config_.obstacle_lon_start_buffer());
    pull_over_status->set_length_back(config_.obstacle_lon_end_buffer());
...
bool PathBoundsDecider::SearchPullOverPosition(
  ...
  const double pull_over_space_length =
      ...
      config_.obstacle_lon_start_buffer() - config_.obstacle_lon_end_buffer();
...
void PathBoundsDecider::GetBoundaryFromLaneChangeForbiddenZone(
  ...
  if (lane_change_status->exist_lane_change_start_position()) {
    ...
    lane_change_start_s = config_.lane_change_prepare_length() + adc_frenet_s_;
...
std::vector<ObstacleEdge> PathBoundsDecider::SortObstaclesForSweepLine(
  ...
  for (const auto* obstacle : indexed_obstacles.Items()) {
    ...
    sorted_obstacles.emplace_back(
        1, obstacle_sl.start_s() - config_.obstacle_lon_start_buffer(),
        obstacle_sl.start_l() - config_.obstacle_lat_buffer(),
        obstacle_sl.end_l() + config_.obstacle_lat_buffer(), obstacle->Id());
    sorted_obstacles.emplace_back(
        0, obstacle_sl.end_s() + config_.obstacle_lon_end_buffer(),
        obstacle_sl.start_l() - config_.obstacle_lat_buffer(),
        obstacle_sl.end_l() + config_.obstacle_lat_buffer(), obstacle->Id());
  }
...
bool PathBoundsDecider::UpdatePathBoundaryWithBuffer(
  ...
  double left_adc_buffer_coeff =
      (is_left_lane_bound ? config_.adc_buffer_coeff()
                          : default_adc_buffer_coeff);
  double right_adc_buffer_coeff =
      (is_right_lane_bound ? config_.adc_buffer_coeff()
                           : default_adc_buffer_coeff);
