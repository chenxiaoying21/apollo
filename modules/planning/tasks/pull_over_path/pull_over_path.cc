...
bool PullOverPath::DecidePathBounds(std::vector<PathBoundary>* boundary) {
  ...
  if (init_sl_state_.second[0] < std::get<1>(path_bound.front()) ||
      init_sl_state_.second[0] > std::get<2>(path_bound.front())) {
  ...
  if (pull_over_type == PullOverStatus::EMERGENCY_PULL_OVER) {
    double adc_to_left_bound =
        std::get<2>(path_bound.front()) - init_sl_state_.second[0];
  ...
  for (const auto& path_boundary : path_boundaries) {
    ...
    if (pull_over_status.has_position() &&
      ...
      if (pull_over_status.pull_over_type() ==
        ...
        weight_ref_l.assign(path_boundary_size,
                            config_.emergency_pull_over_weight());
      } else {
...
void PullOverPath::UpdatePullOverBoundaryByLaneBoundary(
    bool is_pull_over_right, PathBound* const path_bound) {
  const ReferenceLine& reference_line = reference_line_info_->reference_line();
  for (size_t i = 0; i < path_bound->size(); ++i) {
    ...
    if (is_pull_over_right) {
      ...
      std::get<1>((*path_bound)[i]) = right_bound;
    }
  }
}
...
bool PullOverPath::SearchPullOverPosition(
  ...
  if (pull_over_status.pull_over_type() ==
      PullOverStatus::EMERGENCY_PULL_OVER) {
    if (!FindEmergencyPullOverS(&pull_over_s)) {
...
bool PullOverPath::FindEmergencyPullOverS(double* pull_over_s) {
...
bool PullOverPath::FindDestinationPullOverS(
    const std::vector<std::tuple<double, double, double>>& path_bound,
    double* pull_over_s) {
