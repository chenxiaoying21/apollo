...
CreepDecider::CreepDecider(const CreepDeciderConfig& config,
...
Status CreepDecider::Process(Frame* frame,
  ...
  if (stop_line_s > 0.0) {
    ...
    util::BuildStopDecision(
        virtual_obstacle_id, creep_stop_s, config_.stop_distance(),
...
bool CreepDecider::CheckCreepDone(const Frame& frame,
  ...
  const auto& creep_config = config_;
