...
function main() {
    ...
    case "${cmd}" in
        ...
        build_opt_gpu)
            ...
            ;;
        build_opt_gpu_pnc)
            env ${APOLLO_ENV} bash "${build_sh}" --config=opt --config=gpu "cyber planning prediction control routing dreamview task_manager tools common_msgs"
            ;;        
        build_pnc)
            env ${APOLLO_ENV} bash "${build_sh}" --config=dbg --config=gpu "cyber planning prediction control routing dreamview task_manager tools common_msgs"
            ;;
        build_prof)
